...
- `scoped`: named `local`, `ldap`, or `remote` auth providers with separate `identity_scope` and `username` login fields and mreg-issued JWT access tokens for namespace-aware principals
