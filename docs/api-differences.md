...
GET /api/v2/inventory/hosts?limit=50&after=opaque-token
→ next page
```
...
- `scoped`: select a configured `local`, `ldap`, or `remote` provider with `identity_scope` and submit `username` separately, always returning an mreg-issued JWT for a namespace-aware principal
