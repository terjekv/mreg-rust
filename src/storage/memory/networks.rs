...
pub(super) fn get_network_by_cidr_in_state(
    state: &MemoryState,
    cidr: &CidrValue,
) -> Result<Network, AppError> {
...
pub(super) fn delete_network_in_state(
    state: &mut MemoryState,
    cidr: &CidrValue,
) -> Result<(), AppError> {
    let key = cidr.as_str();
...
pub(super) fn list_excluded_ranges_in_state(
    state: &MemoryState,
    network: &CidrValue,
    page: &PageRequest,
...
pub(super) fn list_used_addresses_in_state(
    state: &MemoryState,
    cidr: &CidrValue,
...
pub(super) fn list_unused_addresses_in_state(
    state: &MemoryState,
    cidr: &CidrValue,
    limit: Option<u32>,
...
pub(super) fn count_unused_addresses_in_state(
    state: &MemoryState,
    cidr: &CidrValue,
) -> Result<u64, AppError> {
    let network = state.networks.get(&cidr.as_str()).cloned().ok_or_else(|| {
...
impl NetworkStore for MemoryStorage {
    ...
    async fn update_network(
        ...
        let mut state = self.state.write().await;
        update_network_in_state(&mut state, cidr, command)
    }

    async fn delete_network(&self, cidr: &CidrValue) -> Result<(), AppError> {
        let mut state = self.state.write().await;
        delete_network_in_state(&mut state, cidr)
    }
    ...
    async fn list_excluded_ranges(
        ...
        let state = self.state.read().await;
        list_excluded_ranges_in_state(&state, network, page)
    }
    ...
    async fn list_used_addresses(
        ...
        let state = self.state.read().await;
        list_used_addresses_in_state(&state, cidr)
    }
    ...
    async fn list_unused_addresses(
        ...
        let state = self.state.read().await;
        list_unused_addresses_in_state(&state, cidr, limit)
    }

    async fn count_unused_addresses(&self, cidr: &CidrValue) -> Result<u64, AppError> {
        let state = self.state.read().await;
        count_unused_addresses_in_state(&state, cidr)
    }
}
